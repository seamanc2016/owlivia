...
def get_embedding_model() -> Any:
    """Load the embedding model only when dense retrieval is enabled."""

    if not settings.rag_dense_enabled:
        raise RuntimeError(
            "Dense retrieval is disabled for this environment."
        )
...
def _get_fusion_key(row: pd.Series) -> str:
    ...
    # Prefer filename because the database may contain multiple records
    # that point to the same downloaded PDF.
    file_name = str(
        row.get("file_name", "")
    ).strip().lower()
...
def _merge_missing_metadata(
    ...
    for column in SEARCH_COLUMNS:
        ...
        if (
            not _is_useful_value(current_value)
            and _is_useful_value(incoming_value)
        ):
...
def reciprocal_rank_fusion(
    ...
    for results, rank_column in result_sets:
        ...
        for fallback_rank, (_, row) in enumerate(
            ...
            current_rank = fused[fusion_key].get(
                rank_column
            )
            ...
            if (
                "_distance" in row.index
                and pd.notna(row["_distance"])
            ):
            ...
            if (
                "_score" in row.index
                and pd.notna(row["_score"])
            ):
                score = float(row["_score"])

                current_score = fused[fusion_key].get(
                    "bm25_score"
                )
...
def hybrid_search(
    ...
    """Run configured retrieval and return weighted RRF results."""
